...
def handle_start_help(message):
    bot.send_message(message.chat.id, "Привет! Для того, чтобы добавить мем в базу используй команду /add_memes.")
...
def user_entering_name(message):
    ...
    callback_button = types.InlineKeyboardButton(text="Отменить", callback_data="reset")
    keyboard.add(callback_button)
    bot.send_message(message.chat.id, "Введи ключевые слова для поиска твоего мема через запятую: ", reply_markup=keyboard)
